...
gBooks = books;  ///////////////////               ========     strange way to make sure that after adding new book it will be ok in details  ==========
    ...
    return storageService.put(BOOKS_KEY, x);
...
function addGoogleBook(googleBook) {
  const bookToSave = {
    ...
    "thumbnail": '',
    // "thumbnail": googleBook.volumeInfo.imageLinks.thumbnail || 'imgs/book.png',
    // "thumbnail": googleBook.volumeInfo.imageLinks.thumbnail || 'http://coding-academy.org/books-photos/20.jpg',
    // "thumbnail": googleBook.volumeInfo.imageLinks.smallThumbnail,
    "language": googleBook.volumeInfo.language,
    "listPrice": {
      "amount": 999,
      "currencyCode": "USD",
      "isOnSale": googleBook.saleInfo.saleability
    },
  }
  if (googleBook.volumeInfo.imageLinks) {
    if (googleBook.volumeInfo.imageLinks.thumbnail) {
      bookToSave.thumbnail = googleBook.volumeInfo.imageLinks.thumbnail;
    }
  } else {
    bookToSave.thumbnail = 'imgs/book.png';
    // bookToSave.thumbnail = 'imgs/leather-book-preview.png';
  ...
  storageService.post(BOOKS_KEY, bookToSave)
    .then(()=>{                             ///////////////////               ========     strange way to make sure that after adding new book it will be ok in details  ==========
    ...
    books = [
      {
        ...
        // "reviews":[],
        // "review":["{'name':'haha','rate':4,'review':'blabla'}"],
        // "review": 'hahahahhahahaha',
      },
      {
        "id": "JYOJa2NpSCq",
        "title": "morbi",
